...
uint16_t LCD_cur_width;
uint16_t LCD_cur_height;
...
void ILI9488_Init_GPT(void) {
	printf("LCD initial By GPT start\n");
    // 1. 復位屏幕
    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_1, GPIO_PIN_RESET); // RESET 低電平
    HAL_Delay(10);
    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_1, GPIO_PIN_SET);   // RESET 高電平
    HAL_Delay(120);

    // 2. 初始化命令
    ILI9488_SendCommand(0x3A); // Interface Pixel Format
    ILI9488_SendData(0x55);    // 16-bit RGB565

    ILI9488_SendCommand(0x36); // Memory Access Control
    ILI9488_SendData(0x48);    // 設置掃描方向
...
void ILI9488_Init()
{
	...
	HAL_Delay(10);
	RST_D();
	LCD_cur_width=ILI9488_TFTWIDTH;
	LCD_cur_height=ILI9488_TFTHEIGHT;
	//CS_A();
...
void setScrollArea(uint16_t topFixedArea, uint16_t bottomFixedArea)
{
	...
	ILI9488_SendData((LCD_cur_height - topFixedArea - bottomFixedArea) >> 8);
	ILI9488_SendData(LCD_cur_height - topFixedArea - bottomFixedArea);
...
void drawImage(const uint8_t* img, uint16_t x, uint16_t y, uint16_t w, uint16_t h)
{
	...
	if ((x >= LCD_cur_width) || (y >= LCD_cur_height))
		return;
	if ((x + w - 1) >= LCD_cur_width)
		w = LCD_cur_width - x;
	if ((y + h - 1) >= LCD_cur_height)
		h = LCD_cur_height - y;
...
{
	if ((x < 0) || (x >= LCD_cur_width) || (y < 0) || (y >= LCD_cur_height))
		return;
...
{
	// printf("Fill start\n");
	fillRect(0, 0,  LCD_cur_width, LCD_cur_height, color);
	// printf("Fill end\n");
}

void fillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color) {
    if ((x >= LCD_cur_width) || (y >= LCD_cur_height)) return;
    if ((x + w - 1) >= LCD_cur_width) w = LCD_cur_width - x;
    if ((y + h - 1) >= LCD_cur_height) h = LCD_cur_height - y;

    setAddrWindow(x, y, x + w - 1, y + h - 1);

    uint8_t r = (color & 0xF800) >> 8;   // 紅色
    uint8_t g = (color & 0x07E0) >> 3;   // 綠色
    uint8_t b = (color & 0x001F) << 3;   // 藍色

    uint32_t total_pixels = w * h;
    uint32_t buffer_size = 256; // 每次發送 256 像素數據
    uint8_t buffer[256 * 3];    // 緩衝區大小為 256 像素
    ...
    while (total_pixels > 0) {
        uint32_t pixels_to_send = (total_pixels > buffer_size) ? buffer_size : total_pixels;
        HAL_SPI_Transmit(&hspi2, buffer, pixels_to_send * 3, HAL_MAX_DELAY);
...
void setRotation(uint8_t r)
{
	...
	case 1:
		...
		LCD_cur_width = ILI9488_TFTHEIGHT;
		LCD_cur_height = ILI9488_TFTWIDTH;
		break;
	case 2:
	...
	Game_rotation(LCD_cur_height,LCD_cur_width);
}
...
void testLines(uint8_t color)
{
	...
	int x1, y1, x2, y2, w = LCD_cur_width, h = LCD_cur_height;
